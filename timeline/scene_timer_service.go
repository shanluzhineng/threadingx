...
type ISceneTimerService interface {
	...
	// timerCallback 回调
	// 返回一个timerId
	StartNewOneTimer(delayInterval time.Duration, timerCallback func(*TaskItem)) string

	//启动一个新的计时器(一次性触发的)
	// delayInterval 延时多久后触发回调
	// timerCallback 回调
	// data 回调函数所需的数据
	StartNewOneTimerWithData(delayInterval time.Duration, timerCallback func(*TaskItem), data interface{}) string

	//启动一个新的计时器(一直在运行的)
	StartRecurNewTimer(timerInterval time.Duration, timerCallback func(*TaskItem)) string
...
// 启动一个新的计时器(一次性触发的)
func (s *sceneTimerService) StartNewOneTimer(delayInterval time.Duration, timerCallback func(*TaskItem)) string {
...
// 启动一个新的计时器(一次性触发的)
func (s *sceneTimerService) StartNewOneTimerWithData(delayInterval time.Duration, timerCallback func(*TaskItem), data interface{}) string {
...
// 启动一个新的计时器(一直在运行的)
func (s *sceneTimerService) StartRecurNewTimer(timerInterval time.Duration, timerCallback func(*TaskItem)) string {
